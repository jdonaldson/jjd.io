...
description: "Hospital procurement measures spend to the penny and criticality in adjectives — so the dollars win every argument. HAR puts savings opportunity, supply risk, and analyst attention in one currency, quality-adjusted hours, using baseball's replacement-level trick."
...
$$
\mathrm{queue\_score} =
\frac{E[\Delta\mathrm{HAR} \mid \text{review}]
\times P(\text{action} \mid \text{review})
\times \text{persistence (years)}}
{\text{expected analyst-hours to review}}
$$
...
4. **Put your own labor inside the metric.** A break-even ratio for attention is the difference between a prioritization framework and a to-do list.
5. **Falsify cheap before building expensive.** The first computation should be the one that can tell you to stop.
6. **Ask what breaks when everyone adopts it.** A metric good enough to spread will eventually be the market it was calibrated on. Design for the price-maker case before you're in it.
